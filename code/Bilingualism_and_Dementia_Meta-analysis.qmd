...
For FTD dementia subgroup (see @fig-fig6 for the forest plot), the estimated age at FTD onsets for monolinguals and bilinguals were 58.02 with a 95%Crl \[52.95, 62.84\] and 61.44 with a 95%Crl \[57.01, 64.35\], which were largely lower than the main meta-analytic effect and AD subgroup’s onset. Using avg_comparisons() again, the average difference in marginal effects for bilinguals and monolinguals was 3.76 with a 95%Crl \[1.46, 6.26\]. This showed that bilinguals were 3.76 years older than monolinguals when having the first symptom onset for FTD type of dementia. The heterogeneity level was even lower in this group as since the group-level estimated effects for [monolingual group and bilingual group]{.red} were 2.92 and 1.61. Yet, the relatively low number of studies might have contributed to the low heterogeneity and the wide 95%Crl of the estimated difference between bilinguals and monolinguals at the population level.

```{r}
#Addressing 
# Extract draws
draws_AD  <- as_draws_df(meta_analysismodel_3)
draws_FTD <- as_draws_df(meta_analysismodel_4)

# Compute bilingual - monolingual within each group
effect_AD_diff  <- draws_AD$b_mobibilingual  - draws_AD$b_mobimonolingual
effect_FTD_diff <- draws_FTD$b_mobibilingual - draws_FTD$b_mobimonolingual

# Compute difference between the two subgroups
diff_FTD_AD <- effect_FTD_diff - effect_AD_diff

# Summarize
mean_diff <- mean(diff_FTD_AD)
ci_diff   <- quantile(diff_FTD_AD, probs = c(0.025, 0.975))
prob_FTD_greater <- mean(diff_FTD_AD > 0)
prob_AD_greater  <- mean(diff_FTD_AD < 0)

# Output
cat("Posterior difference in bilingual *advantage* (FTD − AD):\n")
cat("Mean difference:", round(mean_diff, 2), "\n")
cat("95% Credible Interval:", round(ci_diff[1], 2), "to", round(ci_diff[2], 2), "\n")
cat("P(FTD > AD):", round(prob_FTD_greater, 3), "\n")
cat("P(AD > FTD):", round(prob_AD_greater, 3), "\n")
